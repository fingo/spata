...
/** Exception reported for CSV format errors. May be thrown or raised through [[fs2.Stream#raiseError]].
  ...
  * It start with `1` for data, with header row having number `0`.
  * It differs from `line` for sources with header or fields containing line breaks.
  *
  * `col` is the character at given line at which the error has been detected.
  * `0` means "before first character".
  *
  * `field` is the name of field at which error has been detected.
  * It may be a name from header or a number (index starting with `0`) if no header is presented.
  *
