...
final private[spata] class Content[F[_]: Sync: Logger] private (
  ...
  private def wrapRecord(rr: RecordResult): Content.ESE[Record] = rr match
...
private[spata] object Content:
  ...
  /* Convenience type representing result of retrieving CSV content. */
  private type ESE[A] = Either[StructureException, A]
  ...
  def apply[F[_]: Sync: Logger](
    ...
    Header.create(headerSize, headerMap).flatMap(header => Right(new Content(data, header, false)))

  /* Build header for based on header record, remapping selected header values. */
  private def createHeader(rr: RecordResult, headerMap: HeaderMap): ESE[Header] = rr match
