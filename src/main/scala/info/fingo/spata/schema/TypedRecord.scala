...
/** CSV Record representation with type-safe access to its values.
  ...
  * In contract to regular record, the keys are verified at compile time. See [[apply]] method for more.
  * The returned value has already required type, as declared by schema validation:
  * {{{ val id: Int = record("id") }}}
  *
  * Thanks to this, in contrast to regular [[Record]], field access operation through [[apply]]
  * and conversion to case class through [[to]] gives required type directly, without wrapping it in `Either`.
  *
  ...
  */
class TypedRecord[+L <: HList] private[schema] (private[schema] val data: L, val lineNum: Int, val rowNum: Int) {

  /** Gets record value in type-safe manner.
    *
    * The key is verified at compile time and has to be present in record to use them.
    * To achieve this, the provided key is required to have singleton type. This works out of the box for literals,
    ...
    * }}}
    * The returned value has the type which have been declared for given field by schema validation
    * (the proper type is inferred, you do not have to provide it explicitly):
    * {{{ val value: Double = record("value") }}}
    * The formal return type is a bit baffling, because it is fixed depending by calling code.
    *
  ...
  /** Converts typed record to a case class.
    ...
    *
    * Because type record has already all the values properly typed, it may be safely converted to a case class.
    * Assuming, that the record has a field `name` of type `String` and a field birthdate of type `LocalDate`,
    * the conversion is as simple as following:
    * {{{
    ...
    * In such case the field names must match the tuple field naming convention: `_1`, `_2` etc.
    *
...
object TypedRecord {
  ...
  class ToProduct[P <: Product, +L <: HList] private[schema] (data: L) {
    ...
    /** Converts typed record to [[scala.Product]], e.g. case class.
      ...
      * @tparam M the source data type - this is a supertype of record type parameter to overcame covariance
